...
export function isCardapioEditing(mealType){
  ...
  return !store.state.cardapioSalvo[mealType]; // nunca foi salvo ainda => começa em modo edição
}
...
export function renderMenuMealCard(mealType){
  ...
  const color = MENU_MEAL_COLORS[mealType];

  if(editing){
    return `
      <section class="card menu-meal-card">
        <div class="menu-meal-header">
          <div class="menu-meal-icon" style="background:${color};"><i data-lucide="${MEAL_ICONS[mealType]}"></i></div>
        ...
        ${items.length===0 ? '<p class="menu-empty-slots">Nenhum alimento ainda. Adicione o primeiro abaixo.</p>' :
          `<div class="menu-food-list">${items.map(it=>renderMenuFoodRow(mealType, it)).join('')}</div>`}
  ...
  return `
    <section class="card menu-meal-card menu-view-card">
      <button class="menu-view-header" data-toggle-menu="${mealType}">
        <div class="menu-view-icon" style="background:${color};"><i data-lucide="${MEAL_ICONS[mealType]}"></i></div>
      ...
      <div class="menu-view-body${expanded?' open':''}">
        ${checkedItems.length===0 ? '<p class="menu-empty-slots">Nada salvo ainda neste cardápio.</p>' :
          `<div class="menu-food-list">${checkedItems.map(it=>renderMenuFoodRowView(it)).join('')}</div>`}
        <button class="menu-edit-discrete" data-edit-meal="${mealType}"><i data-lucide="pencil"></i>Editar</button>
      </div>
    </section>
  `;
}
export function renderMenuFoodRow(mealType, item){
  return `
    <div class="menu-food-row ${item.checked?'checked':''}" data-toggle-food="${item.uid}" data-meal="${mealType}">
      <span class="mf-check"><i data-lucide="check"></i></span>
      <div class="mf-info">
        <span class="mf-name">${escapeHtml(item.name)}</span>
        <span class="mf-sub">${formatQtyLabel(item)} · ${Math.round(item.kcal)}kcal</span>
      </div>
      <button class="mf-remove" data-remove-food="${item.uid}" data-meal="${mealType}"><i data-lucide="x"></i></button>
    </div>
  `;
}
export function renderMenuFoodRowView(item){
  return `
    <div class="food-row">
      <div class="fr-icon"><i data-lucide="${CATEGORY_ICONS[item.category] || 'utensils'}"></i></div>
      <div class="fr-info">
        <span class="fr-name">${escapeHtml(item.name)}</span>
        <span class="fr-sub">${formatQtyLabel(item)}</span>
      </div>
      <span class="fr-kcal">${Math.round(item.kcal)} kcal</span>
    </div>
  `;
}
