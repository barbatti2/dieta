  ...
  return dates.map(dt=>{
    ...
    return `<div class="hist-day-card">
      ...
      <div class="hist-day-body${expanded?' open':''}">
        ...
        ${groups}
        <button class="hist-edit-day-btn" data-edit-day="${dt}">Editar dia</button>
      </div>
    </div>`;
  }).join('');
}
...
export function wireHistorico(){
  ...
  document.getElementById('histContent').addEventListener('click', (e)=>{
    ...
    const editDayBtn = e.target.closest('[data-edit-day]');
    if(editDayBtn){
      const dt = editDayBtn.dataset.editDay;
      Object.keys(MEAL_LABELS).forEach(mt=>{ histMealExpanded[dt+'|'+mt] = true; });
      document.getElementById('histContent').innerHTML = renderHistContent();
      icons();
    }
  });
