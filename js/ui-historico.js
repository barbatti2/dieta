  ...
  return dates.map(dt=>{
    ...
    return `<div class="hist-day-card">
      <button class="hist-day-header" data-toggle-day="${dt}">
        <div class="hist-day-date"><span class="hd-day">${dayNum}</span><span class="hd-month">${monthAbbr}</span></div>
        ...
        ${dayStatusBadge(total, store.state.goals.calorias)}
      </button>
...
export function dayStatusBadge(kcalTotal, goalKcal){
  if(!goalKcal || goalKcal<=0) return '';
  if(kcalTotal <= goalKcal) return `<span class="day-status ok"><i data-lucide="check-circle-2"></i></span>`;
  const overPct = (kcalTotal-goalKcal)/goalKcal;
  if(overPct <= 0.15) return `<span class="day-status warn"><i data-lucide="alert-circle"></i></span>`;
  return `<span class="day-status over"><i data-lucide="x-circle"></i></span>`;
}
