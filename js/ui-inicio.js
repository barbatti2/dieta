  ...
  return `
    <section class="card today-card">
      <div class="today-card-top">
        ...
        <span class="today-badge">${PROFILE_NAMES[store.currentProfile]}${isToday?'':' · dia selecionado'}</span>
      </div>
      <div class="today-body">
        ...
        <div class="today-macros">
          <div class="today-macro-row">
            <div class="today-macro-label"><span class="tm-name">Proteína <span class="tm-vals">${Math.round(sum.protein)}/${goals.proteina}g</span></span><span class="tm-pct">${Math.round(pctProtein)}%</span></div>
            <div class="today-macro-bar-track"><div class="today-macro-bar-fill" data-fill="${pctProtein}%" style="width:0%;"></div></div>
          </div>
          <div class="today-macro-row">
            <div class="today-macro-label"><span class="tm-name">Carboidratos <span class="tm-vals">${Math.round(sum.carbs)}/${goals.carboidratos}g</span></span><span class="tm-pct">${Math.round(pctCarbs)}%</span></div>
            <div class="today-macro-bar-track"><div class="today-macro-bar-fill" data-fill="${pctCarbs}%" style="width:0%;"></div></div>
          </div>
          <div class="today-macro-row">
            <div class="today-macro-label"><span class="tm-name">Gordura <span class="tm-vals">${Math.round(sum.fat)}/${goalFat}g</span></span><span class="tm-pct">${Math.round(pctFat)}%</span></div>
            <div class="today-macro-bar-track"><div class="today-macro-bar-fill" data-fill="${pctFat}%" style="width:0%;"></div></div>
          </div>
        </div>
      </div>
      ${isOverGoal
