...
import { MEAL_LABELS, PROFILE_NAMES, store } from './state.js';
...
export function renderInicio(){
  ...
  const pctProtein = clamp(goals.proteina>0 ? sum.protein/goals.proteina*100:0,0,100);
  const pctCarbs = clamp(goals.carboidratos>0 ? sum.carbs/goals.carboidratos*100:0,0,100);
  const pctFat = clamp(goalFat>0 ? sum.fat/goalFat*100:0,0,100);
  ...
  return `
    <section class="card today-card">
      <div class="today-eyebrow">${dayLabel} · ${PROFILE_NAMES[store.currentProfile]}${isToday?'':' · dia selecionado'}</div>
      <div class="today-hero">
        <div class="today-num${isOverGoal?' over':''}" data-count-to="${Math.round(sum.kcal)}">0</div>
        <div class="today-num-label">calorias</div>
      </div>
      <div class="today-bar-track"><div class="today-bar-fill${isOverGoal?' over':''}" data-fill="${pct}%" style="width:0%;"></div></div>
      <div class="today-bar-caption">
        ${isOverGoal
          ? `<span class="over-text"><i data-lucide="alert-triangle"></i>${overAmount} kcal acima da meta</span>`
          : `${remaining} kcal restantes de ${goals.calorias}`}
      </div>

      <div class="today-divider"></div>

      <div class="today-macro-row">
        <span class="today-macro-name">Proteína</span>
        <span class="today-macro-bar-track"><span class="today-macro-bar-fill" data-fill="${pctProtein}%" style="width:0%;"></span></span>
        <span class="today-macro-val"><span data-count-to="${Math.round(sum.protein)}">0</span>g</span>
      </div>
      <div class="today-macro-row">
        <span class="today-macro-name">Carboidratos</span>
        <span class="today-macro-bar-track"><span class="today-macro-bar-fill" data-fill="${pctCarbs}%" style="width:0%;"></span></span>
        <span class="today-macro-val"><span data-count-to="${Math.round(sum.carbs)}">0</span>g</span>
      </div>
      <div class="today-macro-row">
        <span class="today-macro-name">Gordura</span>
        <span class="today-macro-bar-track"><span class="today-macro-bar-fill" data-fill="${pctFat}%" style="width:0%;"></span></span>
        <span class="today-macro-val"><span data-count-to="${Math.round(sum.fat)}">0</span>g</span>
      </div>

      <p class="today-caption">${meals.length} ${meals.length===1?'item registrado':'itens registrados'} ${isToday ? 'hoje' : 'nesse dia'}</p>
    </section>
