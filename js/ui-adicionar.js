...
export function renderAdicionar(){
  return `
    ...
    <section class="composer-hero">
      ...
      <div class="category-chips no-scrollbar" id="categoryChips">
        <button class="cat-chip${addFoodState.activeCategory==='Todos'?' active':''}" data-cat="Todos">Todos</button>
        ${Object.keys(CATEGORY_ICONS).map(cat=>`<button class="cat-chip${addFoodState.activeCategory===cat?' active':''}" data-cat="${cat}">${cat}</button>`).join('')}
      </div>
...
export function wireAdicionar(){
  ...
  function runSearch(){
    ...
    const byCategory = addFoodState.activeCategory === 'Todos' ? FOOD_DB : FOOD_DB.filter(f=>f.category===addFoodState.activeCategory);
    if(q.length===0 && addFoodState.activeCategory==='Todos'){ dropdown.classList.add('hidden'); dropdown.innerHTML=''; return; }
    const results = (q.length===0 ? byCategory : byCategory.filter(f=>normalize(f.name).includes(q))).slice(0,8);
    if(results.length===0){
  ...
  document.getElementById('categoryChips').addEventListener('click', (e)=>{
    ...
    if(!btn) return;
    addFoodState.activeCategory = btn.dataset.cat;
    document.querySelectorAll('.cat-chip').forEach(b=>b.classList.toggle('active', b.dataset.cat===addFoodState.activeCategory));
    runSearch();
  });
